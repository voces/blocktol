...
- **Top PB** — the day's record on the PB (best-build) board. A fresh post goes
  ...
  window to a single message rather than a spray of posts — but only while that
  climb is still one uninterrupted burst: a same-holder improvement posts anew
  once the record has been **matched in between** (breaking a shared record is
  its own event) or the post has gone stale (past `PB_RECORD_RESET_MS`, 12h — a
  return visit). The window is anchored at the post (stamped `announced_at`,
  migration v10) and not moved by edits. It runs from `util/pbBoard.ts`'s
  `onPbBuild` — the one hook the run write path calls whenever a build enters
  the PB field (free-play `commitRun`, or a ranked `updateRun` that reaches the
  field top), which _also_ drives the lost-top notifications off the same load
  of the day's bests. The post's message id + last-announced top
  time/holder/holder-count/`announced_at` live in the `pb_announcement` table
  (one row per iteration, migrations v9–v10); `?wait=true` on the POST returns
